...
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[CATR_ASESORIA_GRUPO]
(
    [PK_ASESORIA_GRUPO] [int] IDENTITY(1,1) NOT NULL,
    [FK_ASESOR] INT NOT NULL,
...
GO
ALTER TABLE [dbo].[CATR_ASESORIA_GRUPO] ADD PRIMARY KEY CLUSTERED 
(
	[PK_ASESORIA_GRUPO] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
GO
ALTER TABLE [dbo].[CATR_ASESORIA_GRUPO] ADD  DEFAULT (getdate()) FOR [FECHA_REGISTRO]
GO
ALTER TABLE [dbo].[CATR_ASESORIA_GRUPO]  WITH CHECK ADD CONSTRAINT [fk_asesor_asesoria_grupo] FOREIGN KEY([FK_ASESOR])
REFERENCES [dbo].[users] ([PK_USUARIO])
GO
GO