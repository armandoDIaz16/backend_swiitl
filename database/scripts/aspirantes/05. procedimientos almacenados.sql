...
AS
        ...
        BEGIN
            ...
            ELSE IF(((SELECT ESTADO FROM CAT_USUARIO WHERE CURP=@CURP)=0) OR ((SELECT ESTADO FROM CAT_USUARIO WHERE CURP=@CURP)=5))
                BEGIN
                        ...
                        INSERT INTO CAT_ASPIRANTE (FK_PERIODO, PREFICHA, NUMERO_PREFICHA,PADRE_TUTOR,MADRE,FK_BACHILLERATO,ESPECIALIDAD,PROMEDIO,NACIONALIDAD,FK_CIUDAD,FK_CARRERA_1,FK_CARRERA_2,FK_PROPAGANDA_TECNOLOGICO, FK_UNIVERSIDAD, FK_CARRERA_UNIVERSIDAD,FK_DEPENDENCIA,TRABAJAS_Y_ESTUDIAS, AYUDA_INCAPACIDAD, AVISO_PRIVACIDAD,FK_PADRE,FK_ESTATUS,LENGUA_INDIGENA)
                    ...
                    ,1,@FK_PADRE,1,@LENGUA_INDIGENA);



                    SELECT 3 AS RESPUESTA, PK_USUARIO, CURP, CORREO1, FECHA_REGISTRO FROM  CAT_USUARIO where CURP = @CURP; -- SE ACTUALIZO USUARIO Y SE REGISTRO LA PREFICHA
    ...
    ELSE
        BEGIN
            ...
            INSERT INTO CAT_USUARIO (NOMBRE, PRIMER_APELLIDO, SEGUNDO_APELLIDO, FECHA_NACIMIENTO, SEXO, CURP,
                                     FK_ESTADO_CIVIL, CALLE, NUMERO_EXTERIOR, NUMERO_INTERIOR, FK_COLONIA,
                                     TELEFONO_CASA, TELEFONO_MOVIL, CORREO1, ESTADO, FK_CODIGO_POSTAL, TIPO_USUARIO)
            VALUES (@NOMBRE, @PRIMER_APELLIDO, @SEGUNDO_APELLIDO, @FECHA_NACIMIENTO, @SEXO, @CURP, @FK_ESTADO_CIVIL,
                    @CALLE, @NUMERO_EXTERIOR, @NUMERO_INTERIOR, @FK_COLONIA, @TELEFONO_CASA, @TELEFONO_MOVIL,
                    @CORREO1, 1, (SELECT PK_CODIGO_POSTAL FROM CAT_CODIGO_POSTAL WHERE NUMERO = @CP), 3);
        ...
        ,1,@FK_PADRE,1,@LENGUA_INDIGENA);
            ...
            INSERT INTO PER_TR_ROL_USUARIO (FK_ROL,FK_USUARIO) VALUES ((SELECT PK_ROL FROM PER_CAT_ROL WHERE NOMBRE = 'Aspirante'),@FK_PADRE);
                    ...
                    SELECT 5 AS RESPUESTA, PK_USUARIO, CURP, CORREO1, FECHA_REGISTRO FROM  CAT_USUARIO where CURP = @CURP; -- SE REGISTRO CORRECTAMENTE
        END
        SET NOCOUNT OFF
go
