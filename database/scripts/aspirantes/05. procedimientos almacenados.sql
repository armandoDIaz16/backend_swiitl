...
AS  
    ...
    IF ((SELECT COUNT(users.CURP) FROM CAT_ASPIRANTE LEFT JOIN users ON users.PK_USUARIO = CAT_ASPIRANTE.FK_PADRE WHERE users.CURP = @CURP)<>0)
        BEGIN
            IF ((SELECT COUNT(users.CURP) FROM CAT_ASPIRANTE LEFT JOIN users ON users.PK_USUARIO = CAT_ASPIRANTE.FK_PADRE WHERE FK_PERIODO = @periodo)<>0)
                BEGIN
                    SELECT 1 AS RESPUESTA; -- YA ESTA REGISTRADA ESA CURP EN ESTE PERIODO
                END
            
            ELSE IF ((SELECT COUNT(users.CORREO1) FROM users WHERE CORREO1 = @CORREO1 AND CURP <> @CURP)<>0)
                BEGIN
        ...
        END
    ELSE IF ((SELECT COUNT(users.CORREO1) FROM users WHERE CORREO1 = @CORREO1)<>0)
        BEGIN
    ...
    ELSE
        BEGIN
            ...
            INSERT INTO users (name, PRIMER_APELLIDO, SEGUNDO_APELLIDO,FECHA_NACIMIENTO,SEXO,CURP,FK_ESTADO_CIVIL,CALLE,NUMERO_EXTERIOR,NUMERO_INTERIOR,FK_COLONIA,TELEFONO_CASA,TELEFONO_MOVIL,CORREO1, ESTADO) 
            VALUES (@name, @PRIMER_APELLIDO, @SEGUNDO_APELLIDO, @FECHA_NACIMIENTO, @SEXO, @CURP, @FK_ESTADO_CIVIL, @CALLE, @NUMERO_EXTERIOR, @NUMERO_INTERIOR, @FK_COLONIA, @TELEFONO_CASA, @TELEFONO_MOVIL, @CORREO1, 0);
