...
-- ESTADOS CURSO
    LEFT JOIN CAT_ESTADOS_CURSO EC ON EC.PK_ESTADO_CURSO=CC.ESTADO
--WHERE
    WHERE
        CC.BORRADO=0;
/*FIN VISTA*/
...
--WHERE
    WHERE
        PC.BORRADO=0 AND CC.BORRADO=0 AND PIC.BORRADO=0;
/*fin vista*/
...
--WHERE
    WHERE
        CC.BORRADO=0 AND PIC.BORRADO=0;
/*FIN VISTA*/
...
/* VISTA PARA EL LISTADO DE CONVOCATORIA  */
DROP VIEW IF EXISTS VIEW_CONVOCATORIA_CADO;
go
CREATE VIEW VIEW_CONVOCATORIA_CADO AS
SELECT
    CC.PK_CAT_CURSO_CADO,
    CC.NOMBRE_CURSO,  
    AA.NOMBRE AS NOMBRE_AREA,
    CC.FK_EDIFICIO,
    CC.FK_FICHA_TECNICA_CADO,
    CC.FK_AREA_ACADEMICA,  
    CC.CUPO_ACTUAL,
    CC.CUPO_MAXIMO,
    CC.NOMBRE_ESPACIO,  
    CC.TOTAL_HORAS,
    ...
    CC.RUTA_IMAGEN_CURSO,
    --PERIODO
    PCD.FECHA_INICIO AS FECHA_INICIO_PERIODO,
    PCD.FECHA_FIN AS FECHA_FIN_PERIODO,
    PCD.NOMBRE_PERIODO
FROM CAT_CURSO_CADO CC
--PERIODO CADO
    INNER JOIN CAT_PERIODO_CADO PCD ON  PCD.PK_PERIODO_CADO = CC.FK_PERIODO_CADO
-- AREA ACADEMICA
    LEFT JOIN CAT_AREA_ACADEMICA AA ON AA.PK_AREA_ACADEMICA=CC.FK_AREA_ACADEMICA
--WHERE
    WHERE
